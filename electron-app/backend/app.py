...
# Load feature flags - use resource path helper
...
def adoc_to_s1000d():
    ...
    try:
        ...
        ruby_backend_path = get_resource_path(os.path.join('ruby', ruby_file))
...
def xml_to_html_stream():
    ...
    # Try temp folder first, fallback to app directory
    try:
        log_file = os.path.join(tempfile.gettempdir(), 'xml_to_html_conversion.log')
    except:
        log_file = os.path.join(os.path.dirname(__file__), 'xml_to_html_conversion.log')
    
    def write_log(message):
        """Write to log file and console"""
        print(f"[XML-HTML] {message}")  # Console output
        try:
            with open(log_file, 'a', encoding='utf-8') as log:
                log.write(f"{message}\n")
        except Exception as e:
            print(f"[XML-HTML] Failed to write log: {e}")
    
    write_log(f"\n{'='*80}")
    write_log(f"XML to HTML route called: {datetime.datetime.now()}")
    ...
    saxon_jar = get_resource_path(os.path.join('saxon', 'saxon9he.jar'))
    xsl_stylesheet = get_resource_path(os.path.join('saxon', 'demo3-1.xsl'))
    
    # Validate required files exist
    try:
        with open(log_file, 'a', encoding='utf-8') as log:
            log.write(f"Saxon JAR path: {saxon_jar}\n")
            log.write(f"Saxon JAR exists: {os.path.exists(saxon_jar)}\n")
            log.write(f"XSL path: {xsl_stylesheet}\n")
            log.write(f"XSL exists: {os.path.exists(xsl_stylesheet)}\n")
            
            # Check Java
            java_exe = get_resource_path(os.path.join('tools', 'java', 'bin', 'java.exe'))
            log.write(f"Java path: {java_exe}\n")
            log.write(f"Java exists: {os.path.exists(java_exe)}\n")
    except Exception as e:
    ...
    try:
        with open(log_file, 'a', encoding='utf-8') as log:
            log.write(f"\nProcessing {len(files)} uploaded files\n")
    except:
        pass
    ...
    for file in files:
        ...
        if file and file.filename:
            ...
            try:
                with open(log_file, 'a', encoding='utf-8') as log:
                    log.write(f"  Secure filename: {filename}\n")
                    log.write(f"  Ends with .xml: {filename.lower().endswith('.xml')}\n")
            except:
                pass
            ...
            if filename.lower().endswith('.xml'):
                ...
                try:
                    with open(log_file, 'a', encoding='utf-8') as log:
                        log.write(f"  Saved to: {filepath}\n")
                        log.write(f"  File exists: {os.path.exists(filepath)}\n")
                except:
                    pass
    
    try:
        with open(log_file, 'a', encoding='utf-8') as log:
            log.write(f"\nTotal saved files: {len(saved_files)}\n")
    except:
        pass
    ...
    def convert_xml_to_html(filename, filepath):
        ...
        try:
            with open(log_file, 'a', encoding='utf-8') as log:
                log.write(f"\n{'='*80}\n")
                log.write(f"XML to HTML conversion: {datetime.datetime.now()}\n")
                ...
                log.write(f"Output: {html_filepath}\n")
                log.write(f"Saxon JAR: {saxon_jar} (exists: {os.path.exists(saxon_jar)})\n")
                log.write(f"XSL: {xsl_stylesheet} (exists: {os.path.exists(xsl_stylesheet)})\n")
                
                # Get bundled Java executable
                java_exe = get_resource_path(os.path.join('tools', 'java', 'bin', 'java.exe'))
                if not os.path.exists(java_exe):
                    java_exe = 'java'  # Fallback to system Java
                
                log.write(f"Java: {java_exe}\n")
                
                # Build Saxon command
                saxon_args = [
                    java_exe, '-jar', saxon_jar,
                    f'-s:{filepath}',
                ...
                if os.path.exists(html_filepath):
                    log.write(f"SUCCESS: Output created ({os.path.getsize(html_filepath)} bytes)\n")
                    return (True, filename, html_filename)
                else:
                    log.write(f"ERROR: Output file not created\n")
                    return (False, filename, 'Output file not created')
                    
        except subprocess.CalledProcessError as e:
            with open(log_file, 'a', encoding='utf-8') as log:
                log.write(f"ERROR: subprocess.CalledProcessError\n")
            ...
            return (False, filename, f'Saxon error: {error_msg[:200]}. Log: {log_file}')
        except subprocess.TimeoutExpired:
            with open(log_file, 'a', encoding='utf-8') as log:
                log.write(f"ERROR: Timeout after 120 seconds\n")
            return (False, filename, f'Transformation timed out. Log: {log_file}')
        except FileNotFoundError as e:
            with open(log_file, 'a', encoding='utf-8') as log:
                log.write(f"ERROR: FileNotFoundError\n")
                log.write(f"Error: {str(e)}\n")
                log.write(f"Java command not found in PATH\n")
            return (False, filename, f'Java not found. Check log: {log_file}')
        except Exception as e:
            with open(log_file, 'a', encoding='utf-8') as log:
                log.write(f"ERROR: {type(e).__name__}\n")
                log.write(f"Error: {str(e)}\n")
                import traceback
                log.write(f"Traceback:\n{traceback.format_exc()}\n")
            return (False, filename, f'{str(e)[:200]}. Log: {log_file}')
...
def pm_to_toc():
    ...
    backend_dir = os.path.dirname(os.path.abspath(__file__))
    saxon_jar = get_resource_path(os.path.join('saxon', 'saxon9he.jar'))
    xsl_stylesheet = get_resource_path(os.path.join('saxon', 'PMtoTOC02.xsl'))
